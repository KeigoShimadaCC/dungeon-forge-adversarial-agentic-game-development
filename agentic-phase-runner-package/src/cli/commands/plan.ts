...
import { applyFilePlan, previewPlannedFiles, writePlanPreview } from '../../core/file-plan.js';
import { generateStarterPhasePlan } from '../../core/phase-plan-generator.js';
...
export const runPlanCommand = async (
  ...
  const { report, reportPath } = await applyFilePlan(repoRoot, plan.proposedFiles, { force });
  const skippedFiles = report.files.filter((file) => file.action === 'skipped');
  writeJson({
    schemaVersion: 1,
    status: skippedFiles.length > 0 ? 'applied_with_skips' : 'applied',
    dryRun: false,
    idea: plan.idea,
    reportPath,
    files: report.files,
