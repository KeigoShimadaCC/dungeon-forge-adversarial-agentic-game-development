...
const buildDemoSummaryMarkdown = (result: Omit<DemoLoopResult, 'demoSummaryPath'>): string => {
  const versionLines = result.versions.map((entry) => {
    if (entry.status !== 'completed') {
      return `- ${entry.version}: ${entry.status}${entry.reason ? ` (${entry.reason})` : ''}`;
    }
    const runs = entry.summary?.runs.length ?? 0;
    const balance = entry.balanceSummary?.aggregates;
    const winRate = balance ? `, balance win rate ${balance.win_rate}` : '';
    return `- ${entry.version}: completed, ${runs} summarized runs${winRate}`;
  });
  const comparisonLines = result.comparisons.map((entry) => {
    const items = entry.comparison.objective_metric_deltas.items_used;
    const damage = entry.comparison.objective_metric_deltas.damage_taken;
    return `- ${entry.baseVersion} -> ${entry.targetVersion}: ${entry.comparison.interpretation} items ${items.base}->${items.target}, damage ${damage.base}->${damage.target}`;
  });
...
export const runDemoLoop = async (options: DemoLoopOptions): Promise<DemoLoopResult> => {
  ...
  const demoSummaryPath = path.join('runs', 'demo_summary.md');
  await writeFile(
    path.join(options.runsRoot, demoSummaryPath),
    buildDemoSummaryMarkdown(result),
    'utf8',
  );
  return {
    ...result,
