...
export const worktreePathForPhase = (repoRoot: string, phase: PhaseDefinition): string =>
  path.join(path.dirname(repoRoot), `dungeon-forge-${phaseSlug(phase)}-wt`);
...
const pathScopeConflicts = (left: string, right: string): boolean => {
  if (left === 'PROGRESS.MD' || right === 'PROGRESS.MD') {
    return false;
  }
...
export const validatePhaseGraph = (graph: PhaseGraph): string[] => {
  ...
  for (const phase of graph.phases) {
    for (const dependency of phase.dependsOn) {
      if (!seen.has(dependency) && dependency !== 'PHASE-12A') {
...
export const buildRunnablePhase = (
  ...
  return {
    ...
    codexOrchestrator: {
      role: 'codex',
      canUseCursor: true,
      planPromptTemplate: 'automation/prompts/codex-plan-mode.md',
    },
    cursorDelegate: {
      model: 'composer-2.5',
...
export const buildPhaseRunBundle = async (
  ...
  return {
    ...
    commands: {
      ...
      pr: [
        `gh pr create --fill --base main --head ${quoteShell(runnable.branch)}`,
