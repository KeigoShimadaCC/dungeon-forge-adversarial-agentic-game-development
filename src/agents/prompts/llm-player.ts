...
export const buildLlmPlayerPrompt = (input: LlmPlayerModelInput): string => {
  ...
  return [
    ...
    'Rules:',
    '- Output must be a single JSON object with string fields action_id, action_type, and reason.',
    '- action_id MUST match one of the provided available_actions[].id values.',
    '- action_type MUST match the type of the same available_actions entry as action_id.',
    ...
    '{"action_id":"<id>","action_type":"<type>","reason":"<short explanation>"}',
    'Game input JSON:',
