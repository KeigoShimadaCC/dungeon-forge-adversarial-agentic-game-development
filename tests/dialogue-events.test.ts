...
import type { GameEvent, GameState, PlayerAction, Position } from '../src/game/types.js';
...
const descendOneFloor = (state: GameState): GameState => {
  const onStairs = {
    ...cloneState(state),
    player: {
      ...state.player,
...
describe('Phase 10A dialogue and events', () => {
  ...
  it('fires one-time floor events and preserves narrative through descent', () => {
    ...
    const floorThree = descendOneFloor(floorTwo);
    const floorFour = descendOneFloor(floorThree);
    const floorFive = descendOneFloor(floorFour);
